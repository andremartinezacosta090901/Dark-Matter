...
class RewardHead(nn.Module):  # Works perfectly
    def __init__(self,
        ...
        self.reward_head = model_builder_layer(in_channel=x_dim, hidden_lay=[embedding_dim], output_channel=bins,
                                               norm=True, zeroLastLayer=True, activation='ReLU')
...
class Buffer:  # WORK in progress
    def __init__(self, obs_dim, action, capacity, epsilon=1e-2, alpha=0.1, beta=0.1):
        ...
        self.obs = torch.empty(capacity, obs_dim, dtype=torch.float)
...
class Buffer_without_lambda:  # WORK in progress
    def __init__(self, obs_dim, action, capacity, epsilon=1e-2, alpha=0.1, beta=0.1):
        self.sumtree = Sum_Tree(capacity)
        self.epsilon = epsilon
        self.alpha = alpha
    ...
    def get_priority(self, mismatch_score):
        priority = abs(
            (self.alpha * mismatch_score + self.beta) + self.epsilon
        )
        return float(priority)

    def add(self, transition, mismatch_score):
        action, reward, continued, obs = transition
        self.obs[self.count] = torch.as_tensor(obs)
        self.action[self.count] = torch.as_tensor(action)
        self.reward[self.count] = torch.as_tensor(reward)
        self.continued[self.count] = torch.as_tensor(continued)
        priority = self.get_priority(mismatch_score)
        self.sumtree.add(priority, self.count)
        self.count = (self.count + 1) % self.size
        self.real_size = min(self.real_size + 1, self.size)
    ...
    def update(self, idx, mismatch_score):
        new_priority = self.get_priority(mismatch_score)
        self.sumtree.update(idx, new_priority)