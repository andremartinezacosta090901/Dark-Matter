...
class GLU(nn.Module):  # Gated Liquid Unit :)
    ...
    def forward(self, x, h, t=1.0, reset=None):
        ...
        if features.shape[-1] + h.shape[-1] != self.gate[0].in_features:
            print(f"\n GLU GATE DEBUG")
            print(f"Features shape: {features.shape} (Size: {features.shape[-1]})")
            print(f"Hidden (h) shape: {h.shape} (Size: {h.shape[-1]})")
            print(f"Combined Input: {features.shape[-1] + h.shape[-1]}")
            print(f"Gate Layer Expects: {self.gate[0].in_features}")
            print(f"Gate Weights Shape: {self.gate[0].weight.shape}")
...
class CFC(nn.Module):  # BUILD LIQUID ENHANCED NETWORK FOR CRITIC IT WILL BE ADD WITH COMPUTE LABAMDA
    def __init__(self, input_size, size, norm=True, activation='ReLU', zeroLastLayer=False):
        super().__init__()
        print(f"VALUE HEAD DEBUG: CFC input_size={input_size}")
