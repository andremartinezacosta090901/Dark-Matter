...
for i in range(1, episodes+1):
    ...
    while True:
        ...
        # Random action from LunarLander-v3
        ...
        action_tensor = F.one_hot(torch.tensor(action), num_classes=4).float().unsqueeze(0).to(device)
    ...
    if i > 10:
        ...
        for t in range(imagination_step):
            ...
            # Preparing data
            act = action[0].item()
            act_tensor = F.one_hot(torch.tensor(act), num_classes=4).float().unsqueeze(0).to(device)
